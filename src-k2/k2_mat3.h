...
K2_INLINE xmat3 *xmat3_create(const xmat3 *copy) {
	xmat3 *dest = k2_alloc_type(xmat3);
...
K2_INLINE xmat3 *xmat3_set(const xmat3 *src, xmat3 *dest) {
...
K2_INLINE xmat3 *xmat3_identity(xmat3 *dest) {
	if (!dest) {
...
K2_INLINE xmat3 *xmat3_transpose(xmat3 *mat, xmat3 *dest) {
...
K2_INLINE xmat3 *xquat_to_xmat3(xquat *quat, xmat3 *dest) {
	if (!dest) {
		dest = xmat3_create(NULL);
	}
...
K2_INLINE xvec3 *xmat3_multiply_xvec3(const xmat3 *mat, xvec3 *vec, xvec3 *dest) {
	float *tgt;
	if (! dest) {
		tgt = vec->data;
	} else {
...
K2_INLINE xvec3 xmat3_transform(const xmat3 *m, xvec3 v) {
	xvec3 r;
	xmat3_multiply_xvec3(m, &v, &r);
	return r;
}

K2_INLINE char * xmat3_str(xmat3 *mat, char *buffer, size_t buffer_size) {
