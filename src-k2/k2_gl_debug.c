...
void k2_gl_breakpoint_func() {
    LOG_DEBUG("Set a breakpoint here to pause on GL errors");
}

void k2_gl_log_bound_textures() {
...
#if defined(K2_GLES) || defined(K2_PLATFORM_OSX)

void k2_gl_debug_install() {
	// unsupported
}

#else
K2_INLINE const char *source_type(GLenum type) {
    switch (type) {
...
K2_INLINE const char *type_type(GLenum type) {
    switch (type) {
...
K2_INLINE const char *severity_type(GLenum type) {
    switch (type) {
...
static APIENTRY void k2_gl_debug_callback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const char *message, void *user_param) {
...
void k2_gl_debug_install() {
    ...
    glDebugMessageCallbackARB(k2_gl_debug_callback, NULL);
    GL_DEBUG();
}
