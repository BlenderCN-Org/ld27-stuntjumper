...
K2_INLINE xmat4 *xmat4_create(xmat4 *mat) {
	xmat4 *dest = k2_alloc_type(xmat4);
	if (mat) {
...
K2_INLINE xmat4 *xmat4_set(const xmat4 *src, xmat4 *dest) {
...
K2_INLINE xmat4 *xmat4_identity(xmat4 *dest) {
	dest->data[0] = 1;
	dest->data[1] = 0;
	dest->data[2] = 0;
	dest->data[3] = 0;
...
K2_INLINE xmat4 *xmat4_transpose(xmat4 *src, xmat4 *dest) {
...
K2_INLINE float xmat4_determinant(const xmat4 *mat) {
...
K2_INLINE xmat4 * xmat4_assign(xmat4 *mat, float a00, float a01, float a02, float a03, float a10, float a11, float a12, float a13, float a20, float a21, float a22, float a23, float a30, float a31, float a32, float a33) {
	if (mat == NULL) {
...
K2_INLINE xmat4 *xmat4_inverse(xmat4 *src, xmat4 *dest) {
    float *m = src->data;
    float inv[16], det;
    int i;
    inv[0] = (m[5]  * m[10] * m[15] -
...
K2_INLINE xmat4 *xmat4_to_rotation_xmat4(const xmat4 *src, xmat4 *dest) {
	if (!dest) {
...
K2_INLINE xmat3 *xmat4_to_xmat3(const xmat4 *src, xmat3 *dest) {
	if (!dest) {
...
K2_INLINE xmat3 *xmat4_to_inverse_xmat3(const xmat4 *src, xmat3 *dest) {
...
K2_INLINE xmat4 *xmat4_multiply(xmat4 *mat1, const xmat4 *mat2, xmat4 *dest) {
	if (!dest) {
		dest = mat1;
	}
...
K2_INLINE xvec3 *xmat4_multiply_xvec3(const xmat4 *mat, xvec3 *vec, xvec3 *dest) {
	float *tgt;
	if (!dest) {
		tgt = &vec->data[0];
	} else {
...
K2_INLINE xvec4 *xmat4_multiply_xvec4(const xmat4 *mat, xvec4 *vec, xvec4 *dest) {
	float *tgt;
	if (!dest) {
		tgt = &vec->data[0];
	} else {
...
K2_INLINE xmat4 *xmat4_translate(xmat4 *mat, xvec3 *vec, xmat4 *dest) {
...
K2_INLINE xmat4 *xmat4_scale(xmat4 *mat, xvec3 *vec, xmat4 *dest) {
	float x = vec->data[0], y = vec->data[1], z = vec->data[2];
...
K2_INLINE xmat4 *xmat4_rotate(xmat4 *mat, float angle, const xvec3 *axis, xmat4 *dest) {
...
K2_INLINE xmat4 *xmat4_rotate_x(xmat4 *mat, float angle, xmat4 *dest) {
	float s = sinf(angle),
    c = cosf(angle),
    a10 = mat->data[4],
    a11 = mat->data[5],
...
K2_INLINE xmat4 *xmat4_rotate_y(xmat4 *mat, float angle, xmat4 *dest) {
	float s = sinf(angle),
    c = cosf(angle),
    a00 = mat->data[0],
    a01 = mat->data[1],
...
K2_INLINE xmat4 *xmat4_rotate_z(xmat4 *mat, float angle, xmat4 *dest) {
	float s = sinf(angle),
    c = cosf(angle),
    a00 = mat->data[0],
    a01 = mat->data[1],
...
K2_INLINE xmat4 *xmat4_frustum(float left, float right, float bottom, float top, float fnear, float ffar, xmat4 *dest) {
	if (!dest) {
		dest = xmat4_create(NULL);
	}
	float rl = (right - left),
...
K2_INLINE xmat4 *xmat4_perspective(float fovy, float aspect, float fnear, float ffar, xmat4 *dest) {
	float top = fnear * tanf(fovy * M_PI / 360.0),
    right = top * aspect;
	return xmat4_frustum(-right, right, -top, top, fnear, ffar, dest);
}

K2_INLINE xmat4 *xmat4_ortho(float left, float right, float bottom, float top, float fnear, float ffar, xmat4 *dest) {
	if (!dest) {
		dest = xmat4_create(NULL);
	}
	float rl = (right - left),
...
K2_INLINE xmat4 *xmat4_look_at(const xvec3 *eye, const xvec3 *center, const xvec3 *up, xmat4 *dest) {
	if (!dest) {
		dest = xmat4_create(NULL);
	}
...
K2_INLINE xmat4 *xmat4_from_rotate_translate(const xquat *quat, const xvec3 *vec, xmat4 *dest) {
	if (!dest) {
		dest = xmat4_create(NULL);
	}
...
K2_INLINE xvec4 xvec4_transform(const xmat4 *m, xvec4 v) {
	xvec4 r;
...
K2_INLINE xmat4 *xquat_to_xmat4(const xquat *quat, xmat4 *dest) {
	if (!dest) {
		dest = xmat4_create(NULL);
	}
...
K2_INLINE xmat4 *xmat4_set_rotation_quat(xmat4 *mat, const xquat *quat, xmat4 *dest) {
    if (! dest) {
        dest = mat;
    }
...
K2_INLINE xmat4 *xmat3_to_xmat4(xmat3 *mat, xmat4 *dest) {
	if (!dest) {
		dest = xmat4_create(NULL);
	}
...
K2_INLINE char * xmat4_str(xmat4 *mat, char *buffer, size_t buffer_size) {
