...
void build_extension_cache(void) {
	...
	for (int i = 0; i < extensions_length; ++i) {
		if (extension_string[i] == ' ') {
			...
			extension_cache.extensions[extension_cache.extension_count - 1] = (GLubyte *)strndup((const char *)extension_string[i], i - last_start);
		}
		while (extension_string[i] == ' ' && i < extensions_length) ++i;
	}
#else
